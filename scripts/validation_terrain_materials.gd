...
func _process(_delta: float) -> void:
	if bool(_summary.get("applied", false)):
		set_process(false)
		return
...
func apply_materials_now() -> Dictionary:
	...
	var count := _apply_to_meshes(backend, material)
	var profile := _material_profile_summary()
	_summary = {
		"applied": count > 0,
		"materialized_instances": count,
...
func _apply_to_meshes(node: Node, material: Material) -> int:
	var count := 0
	...
	for child in node.get_children():
		if child is Node:
			count += _apply_to_meshes(child, material)
	return count
