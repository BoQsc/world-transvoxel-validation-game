...
const MARKER := "WT_VALIDATION_G14_GENERATED_64X64_PLAYABLE_STREAMING_PASS"
const PROFILE_ID := &"g14_generated_64x64"
const EXPECTED_PAGE_COUNT := 4096
...
const PATH_SAMPLES := [
	...
	{
		"label": "near_mid",
		"position": Vector3(264, 8, 264),
		"expected_chunks": 25,
		"center_chunk": Vector3i(16, 0, 16),
	},
	{
		"label": "center",
		"position": Vector3(520, 8, 520),
		"expected_chunks": 25,
		"center_chunk": Vector3i(32, 0, 32),
	},
	{
		"label": "far_mid",
		"position": Vector3(776, 8, 264),
		"expected_chunks": 25,
		"center_chunk": Vector3i(48, 0, 16),
	},
	{
		"label": "far_corner",
		"position": Vector3(1016, 8, 1016),
		"expected_chunks": 9,
		"center_chunk": Vector3i(63, 0, 63),
	},
]
...
func _run_test() -> void:
	...
	print("WT_VALIDATION_G14_PROGRESS step=wait_ready")
	if not await _wait_for_ready(scene):
		_fail("G14 profile did not become ready: %s" % str(scene.get_validation_summary()))
		return
	print("WT_VALIDATION_G14_PROGRESS step=wait_initial_floor")
	if not await _wait_for_player_floor(scene):
		_fail("G14 player did not settle on initial terrain: %s" % str(scene.get_validation_summary()))
		return
	var summary: Dictionary = scene.get_validation_summary()
	if not _summary_is_initial_playable(summary):
		_fail("G14 initial playable summary mismatch: %s" % str(summary))
		return
	...
	if reference == null or terrain_world == null:
		_fail("G14 missing reference scene or terrain world")
		return
	...
	if backend_terrain == null or not backend_terrain.has_method("get_world_page_count"):
		_fail("G14 backend terrain cannot report world page count")
		return
	if int(backend_terrain.call("get_world_page_count")) != EXPECTED_PAGE_COUNT:
		_fail("G14 page count mismatch: %s" % str(backend_terrain.call("get_world_page_count")))
		return
	...
	for sample in PATH_SAMPLES:
		print("WT_VALIDATION_G14_PROGRESS step=move label=%s revision=%d" % [sample["label"], revision])
		scene.set("viewer_position", sample["position"])
		if not reference.call("update_reference_viewer", 1, revision, sample["position"], 2, 0):
			_fail("G14 viewer event rejected at %s" % sample["label"])
			return
		if not await _wait_for_window(
			...
			return
		print("WT_VALIDATION_G14_PROGRESS step=window_ready label=%s" % sample["label"])
		...
		if str(sample["label"]) == "center":
			if not await _wait_for_player_floor(scene):
				_fail("G14 player did not settle on center generated terrain: %s" % str(scene.get_validation_summary()))
				return
	...
	if not edit_verified:
		_fail("G14 did not exercise active-center edit")
		return
	if max_render_resources > MAX_ACTIVE_RESOURCES or max_collision_resources > MAX_ACTIVE_RESOURCES:
		_fail("G14 exceeded active resource budget: render=%d collision=%d" % [
...
func _summary_is_initial_playable(summary: Dictionary) -> bool:
	return str(summary.get("playtest_profile_id", "")) == str(PROFILE_ID) and \
			str(summary.get("fixture_label", "")) == "g14_generated_64x64_dense" and \
...
func _move_player_to(scene: Node, viewer_sample_position: Vector3) -> bool:
	...
	if player == null:
		_fail("G14 validation player missing")
		return false
...
func _verify_player_motion(scene: Node, summary: Dictionary) -> bool:
	...
	if before.distance_to(after) < 0.25:
		_fail("G14 scripted player motion did not move: before=%s after=%s" % [str(before), str(after)])
		return false
	return true
...
func _verify_edit(scene: Node, terrain_world: Node, revision: int, center_chunk: Vector3i) -> bool:
	...
	if interactor == null:
		_fail("G14 missing terrain interactor")
		return false
	...
	if not bool(interactor.call("submit_sphere_edit", &"carve", point, 1.8, -1, 1.0)):
		_fail("G14 active-center carve was rejected")
		return false
	if not await _wait_for_revision(terrain_world, start_world_revision + 1):
		_fail("G14 active-center carve did not commit")
		return false
	...
	if int(metrics.get("edit_replacements", 0)) <= start_replacements:
		_fail("G14 active-center carve did not replace any render resource: %s" % str(metrics))
		return false
	return true
...
func _wait_for_window(
	...
	for _frame in range(1500):
		...
		if active_records == expected_chunks and \
			...
			return true
		await process_frame
	_fail("G14 window did not settle at %s expected=%d metrics=%s" % [
		label,
		expected_chunks,
...
func _fail(message: String) -> void:
	push_error("WT_VALIDATION_G14_GENERATED_64X64_PLAYABLE_STREAMING_FAIL: " + message)
	quit(1)
