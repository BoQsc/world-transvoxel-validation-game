...
func _run_test() -> void:
	...
	print("WT_VALIDATION_G19_PROGRESS step=wait_ready")
	if not await _wait_for_ready(scene):
		_fail("G19 profile did not become ready: %s" % str(scene.get_validation_summary()))
		return
	print("WT_VALIDATION_G19_PROGRESS step=wait_initial_floor")
	if not await _wait_for_player_floor(scene):
		_fail("G19 player did not settle on initial terrain: %s" % str(scene.get_validation_summary()))
		return
	if not _assert_no_dense_files("after_start"):
		return
	...
	if reference == null or terrain_world == null:
		_fail("G19 missing reference scene or terrain world")
		return
	...
	if backend_terrain == null or not backend_terrain.has_method("get_world_page_count"):
		_fail("G19 backend terrain cannot report world page count")
		return
	if int(backend_terrain.call("get_world_page_count")) != EXPECTED_PAGE_COUNT:
		_fail("G19 page count mismatch: %s" % str(backend_terrain.call("get_world_page_count")))
		return
	...
	for sample in PATH_SAMPLES:
		print("WT_VALIDATION_G19_PROGRESS step=move label=%s revision=%d" % [sample["label"], revision])
		scene.set("viewer_position", sample["position"])
		if not reference.call("update_reference_viewer", 1, revision, sample["position"], 2, 0):
			_fail("G19 viewer event rejected at %s" % sample["label"])
			return
		if not await _wait_for_window(
			...
			return
		print("WT_VALIDATION_G19_PROGRESS step=window_ready label=%s" % sample["label"])
		...
		if str(sample["label"]) == "center":
			if not await _wait_for_player_floor(scene):
				_fail("G19 player did not settle on center procedural terrain: %s" % str(scene.get_validation_summary()))
				return
	...
	if not edit_verified:
		_fail("G19 did not exercise active-center edit")
		return
	if not _assert_no_dense_files("after_edit"):
		return
	if max_render_resources > MAX_ACTIVE_RESOURCES or max_collision_resources > MAX_ACTIVE_RESOURCES:
		_fail("G19 exceeded active resource budget: render=%d collision=%d" % [
			max_render_resources,
			max_collision_resources,
		])
		return
	var final_metrics: Dictionary = terrain_world.call("get_runtime_metrics")
	print("%s profile=%s samples=%d pages=%d max_render_resources=%d max_collision_resources=%d edit_replacements=%d dense_world_files=0" % [
		MARKER,
		str(PROFILE_ID),
		PATH_SAMPLES.size(),
		EXPECTED_PAGE_COUNT,
...
func _move_player_to(scene: Node, viewer_sample_position: Vector3) -> bool:
	...
	if player == null:
		_fail("G19 validation player missing")
		return false
...
func _verify_edit(scene: Node, terrain_world: Node, revision: int, center_chunk: Vector3i) -> bool:
	...
	if interactor == null:
		_fail("G19 missing terrain interactor")
		return false
	...
	if not bool(interactor.call("submit_sphere_edit", &"carve", point, 1.8, -1, 1.0)):
		_fail("G19 active-center carve was rejected")
		return false
	if not await _wait_for_revision(terrain_world, start_world_revision + 1):
		_fail("G19 active-center carve did not commit")
		return false
...
func _wait_for_window(
	...
	for _frame in range(2400):
		...
		if active_records == expected_chunks and \
			...
			return true
		await process_frame
	_fail("G19 window did not settle at %s expected=%d metrics=%s" % [
		label,
		expected_chunks,
