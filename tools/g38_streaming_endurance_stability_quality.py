...
ARTIFACT_ROOT = ROOT / "artifacts" / "g38_streaming_endurance_stability_quality"
PROJECT = ARTIFACT_ROOT / "project"
SCRIPT = "res://tests/g38_streaming_endurance_stability_quality.gd"
MARKER = "WT_VALIDATION_G38_STREAMING_ENDURANCE_STABILITY_PASS"
SUMMARY_MARKER = "WT_VALIDATION_G38_STREAMING_ENDURANCE_STABILITY_SMOKE_PASS"
...
def run_runtime(project: Path, version: str, engine: Path) -> dict[str, object]:
    ...
    (ARTIFACT_ROOT / f"godot-{version}-g38.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (ARTIFACT_ROOT / f"godot-{version}-g38.stderr.txt").write_text(result.stderr, encoding="utf-8")
    combined = result.stdout + result.stderr
    print(combined, end="" if combined.endswith("\n") else "\n")
    if result.returncode != 0 or MARKER not in combined or has_godot_error(combined):
        raise RuntimeError(f"G38 streaming endurance stability quality failed on {version}")
    if duration_seconds > MAX_RUNTIME_SECONDS:
        raise RuntimeError(f"G38 runtime exceeded {MAX_RUNTIME_SECONDS:.0f}s on {version}: {duration_seconds:.3f}s")
    ...
    if fields.get("profile") != PROFILE_ID or fields.get("dense_world_files") != "0":
        raise RuntimeError(f"G38 marker fields invalid: {marker_line}")
    if fields.get("final_cold_idle") != "true":
        raise RuntimeError(f"G38 final cold-idle marker invalid: {marker_line}")
    for field in ("max_render_resources", "max_collision_resources", "max_active_records"):
        if int(fields.get(field, "999")) > 50:
            raise RuntimeError(f"G38 transition resource field exceeded budget: {field} in {marker_line}")
    for field in ("final_render_resources", "final_collision_resources", "final_active_records"):
        if int(fields.get(field, "0")) != 25:
            raise RuntimeError(f"G38 final resource field did not return to 25: {field} in {marker_line}")
    if int(fields.get("route_samples", "0")) != 10 or int(fields.get("local_motion_samples", "0")) != 10:
        raise RuntimeError(f"G38 route coverage invalid: {marker_line}")
    if float(fields.get("total_player_motion", "0")) < 50.0:
        raise RuntimeError(f"G38 total player motion too small: {marker_line}")
    return {
        "engine": version,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run streaming endurance stability quality validation.")
    ...
    report_path = ARTIFACT_ROOT / "g38_streaming_endurance_stability_quality_report.json"
    ...
    report_path.write_text(json.dumps({
        ...
        "max_settle_frames": max_settle_frames, "min_total_player_motion": min_total_motion,
        "max_duration_seconds": max_duration, "implementation": "streaming_endurance_stability_quality",
    }, indent=2) + "\n", encoding="utf-8")
    print(
        f"{SUMMARY_MARKER} profile={PROFILE_ID} engines={len(results)} route_cycles=2 route_samples=10 "
