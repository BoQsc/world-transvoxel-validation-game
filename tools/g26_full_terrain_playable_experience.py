...
ARTIFACT_ROOT = ROOT / "artifacts" / "g26_full_terrain_playable_experience"
SCRIPT = "res://tests/g26_full_terrain_playable_experience.gd"
MARKER = "WT_VALIDATION_G26_FULL_TERRAIN_PLAYABLE_EXPERIENCE_PASS"
SUMMARY_MARKER = "WT_VALIDATION_G26_FULL_TERRAIN_PLAYABLE_EXPERIENCE_SMOKE_PASS"
...
def run_runtime(project: Path, version: str, engine: Path) -> dict[str, object]:
    ...
    (ARTIFACT_ROOT / f"godot-{version}-g26.stdout.txt").write_text(
        result.stdout, encoding="utf-8"
    )
    (ARTIFACT_ROOT / f"godot-{version}-g26.stderr.txt").write_text(
        result.stderr, encoding="utf-8"
    )
    combined = result.stdout + result.stderr
    print(combined, end="" if combined.endswith("\n") else "\n")
    if result.returncode != 0 or MARKER not in combined or has_godot_error(combined):
        raise RuntimeError(f"G26 full terrain playable experience failed on {version}")
    if duration_seconds > MAX_RUNTIME_SECONDS:
        raise RuntimeError(
            f"G26 exceeded runtime ceiling on {version}: "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run first-person full-terrain playable experience validation."
    )
    ...
    if max_engine_seconds > MAX_LOAD_TO_PLAY_SECONDS:
        raise RuntimeError(
            "G26 max engine time exceeded load-to-play ceiling: "
            f"{max_engine_seconds:.3f}s > {MAX_LOAD_TO_PLAY_SECONDS:.3f}s"
        )
    report_path = ARTIFACT_ROOT / "g26_full_terrain_playable_experience_report.json"
        ...
        json.dumps(
            {
                ...
                "engines": results,
                "implementation": "full_terrain_playable_experience",
            },
            indent=2,
        )
        + "\n",
