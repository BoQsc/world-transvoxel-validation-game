...
ARTIFACT_ROOT = ROOT / "artifacts" / "g25_full_terrain_visual_baseline"
SCRIPT = "res://tests/g25_full_terrain_visual_baseline.gd"
MARKER = "WT_VALIDATION_G25_FULL_TERRAIN_VISUAL_BASELINE_PASS"
SUMMARY_MARKER = "WT_VALIDATION_G25_FULL_TERRAIN_VISUAL_BASELINE_SMOKE_PASS"
...
def run_runtime(project: Path, version: str, engine: Path) -> dict[str, object]:
    ...
    (ARTIFACT_ROOT / f"godot-{version}-g25.stdout.txt").write_text(
        result.stdout, encoding="utf-8"
    )
    (ARTIFACT_ROOT / f"godot-{version}-g25.stderr.txt").write_text(
        result.stderr, encoding="utf-8"
    )
    combined = result.stdout + result.stderr
    print(combined, end="" if combined.endswith("\n") else "\n")
    if result.returncode != 0 or MARKER not in combined or has_godot_error(combined):
        raise RuntimeError(f"G25 full terrain visual baseline failed on {version}")
    if duration_seconds > MAX_RUNTIME_SECONDS:
        raise RuntimeError(
            f"G25 exceeded runtime ceiling on {version}: "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run full 2048 by 2048 terrain visual coverage validation."
    )
    ...
    if max_engine_seconds > MAX_LOAD_TO_PLAY_SECONDS:
        raise RuntimeError(
            "G25 max engine time exceeded load-to-play ceiling: "
            f"{max_engine_seconds:.3f}s > {MAX_LOAD_TO_PLAY_SECONDS:.3f}s"
        )
    report_path = ARTIFACT_ROOT / "g25_full_terrain_visual_baseline_report.json"
        ...
        json.dumps(
            {
                ...
                "engines": results,
                "implementation": "full_terrain_visual_baseline",
            },
            indent=2,
        )
        + "\n",
