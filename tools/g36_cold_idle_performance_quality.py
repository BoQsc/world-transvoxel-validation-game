...
ARTIFACT_ROOT = ROOT / "artifacts" / "g36_cold_idle_performance_quality"
PROJECT = ARTIFACT_ROOT / "project"
SCRIPT = "res://tests/g36_cold_idle_performance_quality.gd"
MARKER = "WT_VALIDATION_G36_COLD_IDLE_PERFORMANCE_PASS"
SUMMARY_MARKER = "WT_VALIDATION_G36_COLD_IDLE_PERFORMANCE_SMOKE_PASS"
...
def run_runtime(project: Path, version: str, engine: Path) -> dict[str, object]:
    ...
    (ARTIFACT_ROOT / f"godot-{version}-g36.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (ARTIFACT_ROOT / f"godot-{version}-g36.stderr.txt").write_text(result.stderr, encoding="utf-8")
    combined = result.stdout + result.stderr
    print(combined, end="" if combined.endswith("\n") else "\n")
    if result.returncode != 0 or MARKER not in combined or has_godot_error(combined):
        raise RuntimeError(f"G36 cold idle performance quality failed on {version}")
    if duration_seconds > MAX_RUNTIME_SECONDS:
        raise RuntimeError(f"G36 runtime exceeded {MAX_RUNTIME_SECONDS:.0f}s on {version}: {duration_seconds:.3f}s")
    marker_line = next(line for line in combined.splitlines() if line.startswith(MARKER))
    fields = parse_marker(marker_line)
    required_zero_fields = (
        "viewer_update_delta",
        "edit_replacement_delta",
    ...
    if fields.get("profile") != PROFILE_ID or fields.get("dense_world_files") != "0":
        raise RuntimeError(f"G36 marker fields invalid: {marker_line}")
    if fields.get("cold_idle") != "true":
        raise RuntimeError(f"G36 cold idle marker invalid: {marker_line}")
    for field in required_zero_fields:
        if int(fields.get(field, "-1")) != 0:
            raise RuntimeError(f"G36 idle churn field is nonzero: {field} in {marker_line}")
    for field in ("max_render_resources", "max_collision_resources", "max_active_records"):
        if int(fields.get(field, "999")) > 25:
            raise RuntimeError(f"G36 active resource field exceeded budget: {field} in {marker_line}")
    return {
        "engine": version,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run cold-idle performance quality validation.")
    ...
    report_path = ARTIFACT_ROOT / "g36_cold_idle_performance_quality_report.json"
        ...
        json.dumps(
            {
                ...
                "implementation": "cold_idle_performance_quality",
            },
            indent=2,
        )
        + "\n",
    ...
    print(
        f"{SUMMARY_MARKER} profile={PROFILE_ID} engines={len(results)} idle_frames=300 "
        "viewer_update_delta=0 edit_replacement_delta=0 material_auto_apply_delta=0 "
        "max_render_resources=25 max_collision_resources=25 max_active_records=25 "
