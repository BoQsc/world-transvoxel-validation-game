...
ARTIFACT_ROOT = ROOT / "artifacts" / "g16_generated_128x128_playable_streaming"
SOURCE_ROOT = ARTIFACT_ROOT / "source"
WORLDS_ROOT = ARTIFACT_ROOT / "worlds"
G16_PROJECT = ARTIFACT_ROOT / "project"
PROFILE_ID = "g16_generated_128x128"
WORLD_OUTPUT = WORLDS_ROOT / PROFILE_ID
WORLD_MANIFEST = WORLD_OUTPUT / "world.wtworld"
PROJECT_WORLD_ROOT = Path("build") / "g16-generated-fixture" / PROFILE_ID
SCRIPT = "res://tests/g16_generated_128x128_playable_streaming_smoke.gd"
MARKER = "WT_VALIDATION_G16_GENERATED_128X128_PLAYABLE_STREAMING_PASS"
...
def height(x: int, z: int) -> float:
    centered_x = (x - 1023.5) / 1023.5
    centered_z = (z - 1023.5) / 1023.5
    ridge = 3.2 * math.exp(-3.0 * (centered_x * centered_x + centered_z * centered_z))
    long_wave = 0.80 * math.sin(x * 0.018) + 0.60 * math.cos(z * 0.016)
    diagonal = 0.40 * math.sin((x + z) * 0.008)
    local = 0.28 * math.cos((x - z) * 0.021)
...
def source_summary() -> dict[str, int]:
    density = SOURCE_ROOT / "density.f32"
    materials = SOURCE_ROOT / "materials.u16"
    keys = SOURCE_ROOT / "keys.txt"
    for path in (density, materials, keys):
        if not path.is_file():
            raise RuntimeError(f"G16 source artifact missing: {path}")
    density_size = density.stat().st_size
    material_size = materials.stat().st_size
    if density_size != EXPECTED_DENSITY_BYTES:
        raise RuntimeError(f"G16 density source size mismatch: {density_size} != {EXPECTED_DENSITY_BYTES}")
    if material_size != EXPECTED_MATERIAL_BYTES:
        raise RuntimeError(f"G16 material source size mismatch: {material_size} != {EXPECTED_MATERIAL_BYTES}")
    key_lines = keys.read_text(encoding="utf-8").splitlines()
    if len(key_lines) != len(CHUNK_KEYS):
        raise RuntimeError(f"G16 key source count mismatch: {len(key_lines)} != {len(CHUNK_KEYS)}")
    return {
...
def bake_fixture(reuse_fixture: bool) -> dict[str, object]:
    ...
    if result.returncode != 0 or not WORLD_MANIFEST.is_file():
        raise RuntimeError("G16 generated 128x128 fixture bake failed")
...
def validate_fixture(reused: bool) -> dict[str, object]:
    ...
    if metadata.get("pages") != len(CHUNK_KEYS):
        raise RuntimeError(f"G16 generated 128x128 page count mismatch: {metadata}")
    if metadata.get("source_revision") != SOURCE_REVISION:
        raise RuntimeError(f"G16 generated 128x128 source revision mismatch: {metadata}")
    return {
        "reused": reused,
        "profile": PROFILE_ID,
...
def run_smoke(project: Path, version: str, engine: Path, headless: bool) -> dict[str, object]:
    ...
    (ARTIFACT_ROOT / f"godot-{version}-g16.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (ARTIFACT_ROOT / f"godot-{version}-g16.stderr.txt").write_text(result.stderr, encoding="utf-8")
    combined = result.stdout + result.stderr
    print(combined, end="" if combined.endswith("\n") else "\n")
    if result.returncode != 0 or MARKER not in combined or has_godot_error(combined):
        raise RuntimeError(f"G16 generated 128x128 playable streaming smoke failed on {version}")
    return {
        "engine": version,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Bake and run the G16 generated 128x128 playable streaming validation smoke."
    )
    ...
    report_path = ARTIFACT_ROOT / "g16_generated_128x128_playable_streaming_report.json"
        ...
        json.dumps(
            {
                ...
                "fixture": fixture,
                "engines": results,
                "implementation": "generated_128x128_playable_streaming",
            },
            indent=2,
        )
        + "\n",
