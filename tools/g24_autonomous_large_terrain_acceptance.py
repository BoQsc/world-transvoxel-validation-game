...
ARTIFACT_ROOT = ROOT / "artifacts" / "g24_autonomous_large_terrain_acceptance"
SCRIPT = "res://tests/g24_autonomous_large_terrain_acceptance.gd"
MARKER = "WT_VALIDATION_G24_AUTONOMOUS_LARGE_TERRAIN_ACCEPTANCE_PASS"
SUMMARY_MARKER = "WT_VALIDATION_G24_AUTONOMOUS_LARGE_TERRAIN_ACCEPTANCE_SMOKE_PASS"
...
def run_runtime(project: Path, version: str, engine: Path) -> dict[str, object]:
    ...
    (ARTIFACT_ROOT / f"godot-{version}-g24.stdout.txt").write_text(
        result.stdout, encoding="utf-8"
    )
    (ARTIFACT_ROOT / f"godot-{version}-g24.stderr.txt").write_text(
        result.stderr, encoding="utf-8"
    )
    combined = result.stdout + result.stderr
    print(combined, end="" if combined.endswith("\n") else "\n")
    if result.returncode != 0 or MARKER not in combined or has_godot_error(combined):
        raise RuntimeError(f"G24 autonomous large-terrain acceptance failed on {version}")
    if duration_seconds > MAX_RUNTIME_SECONDS:
        raise RuntimeError(
            f"G24 exceeded runtime ceiling on {version}: "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run autonomous large-terrain acceptance for the compact 2K profile."
    )
    ...
    if max_engine_seconds > MAX_LOAD_TO_PLAY_SECONDS:
        raise RuntimeError(
            "G24 max engine time exceeded load-to-play ceiling: "
            f"{max_engine_seconds:.3f}s > {MAX_LOAD_TO_PLAY_SECONDS:.3f}s"
        )
    report_path = ARTIFACT_ROOT / "g24_autonomous_large_terrain_acceptance_report.json"
        ...
        json.dumps(
            {
                ...
                "engines": results,
                "implementation": "autonomous_large_terrain_acceptance",
            },
            indent=2,
        )
        + "\n",
